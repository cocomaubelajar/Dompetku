...
// ==================== RENDER ====================
function render(){
  const views={dashboard:renderDashboard,transaksi:renderTransaksi,anggaran:renderAnggaran,tabungan:renderTabungan,utang:renderUtang,investasi:renderInvestasi,laporan:renderLaporan};
  const fn=views[currentView];
  document.getElementById('view-container').innerHTML=fn?fn():'';
}
...
function computed(){
  ...
  const netWorth=totalAssets+totalSaved-totalOwed;
  const warnings=budStat.filter(b=>b.pct>=80);
  return{fMonth,transactions,monthTx,income,expense,byCatArr,budStat,investments,savings,debts,totalAssets,totalSaved,totalOwed,totalRec,netWorth,warnings};
}

// ==================== DASHBOARD ====================
function renderDashboard(){
  const{fMonth,monthTx,income,expense,byCatArr,budStat,netWorth,warnings}=computed();
  ...
  const txRows=recentTx.map(tx=>{
    ...
    return`<div class="tx-row">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div class="muted" style="font-size:11px">${tx.date}${cat?` · ${cat.label}`:''}</div>
      </div>
  ...
  const cw=480,ch=160,pad=30,barW=18,gap=8;
  const barsHtml=last6.map((d,i)=>{
    const x=pad+i*(barW*2+gap+12);
    const incH=Math.round((d.inc/maxVal)*(ch-40));
    const expH=Math.round((d.exp/maxVal)*(ch-40));
    return`<rect x="${x}" y="${ch-20-incH}" width="${barW}" height="${incH}" fill="#1D9E75" rx="3"/>
    <rect x="${x+barW+2}" y="${ch-20-expH}" width="${barW}" height="${expH}" fill="#D85A30" rx="3"/>
    <text x="${x+barW+1}" y="${ch-5}" text-anchor="middle" font-size="10" fill="var(--text3)">${d.name}</text>`;
  }).join('');

  const yLabels=[0,0.5,1].map(p=>{
    const val=(maxVal*p);
    const y=ch-20-Math.round(p*(ch-40));
    return`<text x="${pad-5}" y="${y}" text-anchor="end" font-size="9" fill="var(--text3)">${val.toFixed(1)}</text>
    <line x1="${pad}" y1="${y}" x2="${cw-10}" y2="${y}" stroke="var(--border)" stroke-width="0.5"/>`;
  }).join('');
  ...
  if(byCatArr.length>0){
    ...
    const slices=byCatArr.map(c=>{
      ...
      const si=polarToXY(cx,cy,ri,startA);
      const ei=polarToXY(cx,cy,ri,endA);
      return`<path d="M ${s.x} ${s.y} A ${r} ${r} 0 ${large} 1 ${e.x} ${e.y} L ${ei.x} ${ei.y} A ${ri} ${ri} 0 ${large} 0 ${si.x} ${si.y} Z" fill="${c.color}" opacity=".9"/>`;
    }).join('');
    const legendItems=byCatArr.slice(0,5).map(c=>`<span style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--text2)"><span style="width:8px;height:8px;border-radius:2px;background:${c.color};display:inline-block;flex-shrink:0"></span>${c.name}</span>`).join('');
    donutHtml=`<svg width="140" height="140" style="display:block;margin:0 auto">${slices}</svg><div style="display:flex;flex-wrap:wrap;gap:4px 10px;margin-top:4px">${legendItems}</div>`;
  }
  ...
  <div class="g4" style="margin-bottom:14px">
    <div class="metric"><div class="metric-label">Pemasukan</div><div class="metric-val" style="color:var(--green)">${fmtIDR(income)}</div></div>
    <div class="metric"><div class="metric-label">Pengeluaran</div><div class="metric-val" style="color:var(--red)">${fmtIDR(expense)}</div></div>
    <div class="metric"><div class="metric-label">Saldo Bulan Ini</div><div class="metric-val" style="color:${saldo>=0?'var(--green)':'var(--red)'}">${fmtIDR(saldo)}</div></div>
    <div class="metric"><div class="metric-label">Net Worth</div><div class="metric-val" style="color:var(--blue)">${fmtIDR(netWorth)}</div></div>
  </div>
  <div class="g2">
    <div class="card"><div class="h3" style="margin-bottom:8px">6 Bulan Terakhir <span class="muted" style="font-size:11px">(juta Rp)</span></div>
      <div class="chart-wrap"><svg viewBox="0 0 490 165" width="100%" style="overflow:visible">${yLabels}${barsHtml}</svg></div>
      <div style="display:flex;gap:12px;margin-top:4px"><span style="font-size:11px;color:var(--text2);display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:2px;background:#1D9E75;display:inline-block"></span>Masuk</span><span style="font-size:11px;color:var(--text2);display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:2px;background:#D85A30;display:inline-block"></span>Keluar</span></div>
    </div>
    <div class="card"><div class="h3" style="margin-bottom:8px">Pengeluaran per Kategori</div>${donutHtml}</div>
  </div>
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><div class="h3">Transaksi Terbaru</div><button class="btn" style="font-size:12px" onclick="nav('transaksi')">Lihat semua →</button></div>
...
function renderTransaksi(){
  ...
  return`<div class="row">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn" onclick="exportCSV()">⬇️ Ekspor CSV</button><button class="btn-p" onclick="openModal('tx')">+ Tambah</button></div>
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
    ...
    <select class="sel" style="width:auto" onchange="ss('dk_ftype',this.value);render()"><option value="all"${filterType==='all'?' selected':''}>Semua Tipe</option><option value="income"${filterType==='income'?' selected':''}>Pemasukan</option><option value="expense"${filterType==='expense'?' selected':''}>Pengeluaran</option></select>
    <select class="sel" style="width:auto" onchange="ss('dk_fcat',this.value);render()"><option value="all"${filterCat==='all'?' selected':''}>Semua Kategori</option>${catOpts}</select>
  </div>
  <div class="card">${rows||'<div class="empty">Tidak ada transaksi ditemukan</div>'}</div>`;
}
...
function renderAnggaran(){
  ...
  const cards=budStat.map(b=>`<div class="card">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px"><div><div style="font-size:13px;font-weight:500;display:flex;align-items:center;gap:5px"><span style="width:8px;height:8px;border-radius:2px;background:${b.cat?.color||'#888'};display:inline-block"></span>${b.cat?.label||b.category}</div><div class="muted" style="font-size:11px;margin-top:2px">${fmtFull(b.spent)} / ${fmtFull(b.limit)}</div></div><div style="display:flex;align-items:center;gap:4px">${b.pct>=80?'<span style="font-size:13px">⚠️</span>':''}<span style="font-size:13px;font-weight:600;color:${b.pct>=100?'var(--red)':b.pct>=80?'var(--amber)':'var(--text)'}">${Math.round(b.pct)}%</span><button class="btn-icon" style="font-size:12px" onclick="deleteBudget('${b.id}')">✕</button></div></div>
  ...
  return`<div class="row"><h2 class="h2">Anggaran</h2><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap"><input type="month" class="inp" style="width:140px;font-size:13px" value="${fMonth}" onchange="setMonth(this.value)"><button class="btn-p" onclick="openBudget('')">+ Set Anggaran</button></div></div>
...
function renderTabungan(){
  ...
  const cards=savings.map(sv=>{
    ...
    if(sv.deadline){
      ...
      if(!isNaN(days)){
        deadline=`<div style="font-size:11px;color:${days<0?'var(--red)':days<30?'var(--amber)':'var(--text2)'}">📅 Deadline: ${sv.deadline}${days>=0?` (${days} hari lagi)`:' (lewat)'}</div>`;
      } else {
        deadline=`<div style="font-size:11px;color:var(--text2)">📅 Deadline: ${sv.deadline}</div>`;
      }
    }
    return`<div class="card"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px"><div><div style="font-size:14px;font-weight:500;display:flex;align-items:center;gap:5px">${done?'<span style="color:var(--green)">✓</span>':''}${esc(sv.name)}</div>${deadline}</div><div style="display:flex;gap:2px"><button class="btn-icon" onclick="editSaving('${sv.id}')">✏️</button><button class="btn-icon" style="color:var(--red)" onclick="deleteSaving('${sv.id}')">🗑️</button></div></div>
...
function renderUtang(){
  ...
  function debtRows(items, debtType){
    ...
    const isUtang = debtType === 'utang';
    const colorVar = isUtang ? 'var(--red)' : 'var(--green)';
    const bgVar = isUtang ? 'var(--red-bg)' : 'var(--green-bg)';
    return items.map(d=>{
      let dueInfo='';
      if(d.dueDate){
...
function renderLaporan(){
  ...
  const tableRows=monthly.map((m,i)=>`<tr style="opacity:${(m.inc===0&&m.exp===0) ? 0.3 : 1};border-bottom:1px solid var(--border)"><td style="padding:0.4rem 0.5rem;font-size:13px">${m.name}</td><td style="padding:0.4rem 0.5rem;text-align:right;font-size:13px;color:var(--green)">${fmtIDR(m.inc)}</td><td style="padding:0.4rem 0.5rem;text-align:right;font-size:13px;color:var(--red)">${fmtIDR(m.exp)}</td><td style="padding:0.4rem 0.5rem;text-align:right;font-size:13px;font-weight:500;color:${m.saldo>=0?'var(--green)':'var(--red)'}">${fmtIDR(m.saldo)}</td></tr>`).join('');
...
function renderModal(){
  const titles={tx:editId?'Edit Transaksi':'Tambah Transaksi',budget:'Set Anggaran',saving:editId?'Edit Target':'Target Tabungan Baru',debt:`Catat ${tmpForm.type==='utang'?'Utang':'Piutang'}`,inv:editId?'Edit Investasi':'Tambah Investasi',rates:'Atur Kurs Mata Uang'};
  ...
  if(currentModalType==='tx'){
    const isIncome=(tmpForm.type||'expense')==='income';
    const amtIDR=tmpForm.amount&&tmpForm.currency!=='IDR'?`<div style="font-size:11px;color:var(--text2);margin-top:3px">≈ ${fmtFull(toIDR(tmpForm.amount,tmpForm.currency))}</div>`:'';
    body=`<div class="fg"><label>Tipe</label><div style="display:flex;gap:6px"><button type="button" onclick="sf('type','income')" style="flex:1;padding:0.4rem;border-radius:var(--radius);border:1px solid ${isIncome?'var(--green2)':'var(--border2)'};background:${isIncome?'var(--green-bg)':'transparent'};color:${isIncome?'var(--green)':'var(--text2)'};cursor:pointer;font-size:13px">↑ Pemasukan</button><button type="button" onclick="sf('type','expense')" style="flex:1;padding:0.4rem;border-radius:var(--radius);border:1px solid ${!isIncome?'var(--red2)':'var(--border2)'};background:${!isIncome?'var(--red-bg)':'transparent'};color:${!isIncome?'var(--red)':'var(--text2)'};cursor:pointer;font-size:13px">↓ Pengeluaran</button></div></div>
    <div class="fg"><label>Nominal & Mata Uang</label><div style="display:grid;grid-template-columns:2fr 1fr;gap:6px"><input class="inp" type="number" placeholder="0" value="${tmpForm.amount||''}" oninput="sf('amount',this.value)"><select class="sel" onchange="sf('currency',this.value)">${currOpts}</select></div>${amtIDR}</div>
    ${!isIncome?`<div class="fg"><label>Kategori</label><select class="sel" onchange="sf('category',this.value)">${catOpts}</select></div>`:''}
    ...
    body=`<div class="fg"><label>Kategori</label><select class="sel" onchange="sf('category',this.value)">${catOpts}</select></div><div class="fg"><label>Batas Anggaran (IDR)</label><input class="inp" type="number" placeholder="0" value="${tmpForm.limit||''}" oninput="sf('limit',this.value)"></div>`;
  }else if(currentModalType==='saving'){
    body=`<div class="fg"><label>Nama Target</label><input class="inp" placeholder="Dana Darurat, Liburan, Gadget baru..." value="${esc(tmpForm.name||'')}" oninput="sf('name',this.value)"></div><div class="fg"><label>Target (IDR)</label><input class="inp" type="number" placeholder="0" value="${tmpForm.target||''}" oninput="sf('target',this.value)"></div><div class="fg"><label>Sudah Terkumpul (IDR)</label><input class="inp" type="number" placeholder="0" value="${tmpForm.current||'0'}" oninput="sf('current',this.value)"></div><div class="fg"><label>Deadline (opsional)</label><input class="inp" type="date" value="${tmpForm.deadline||''}" oninput="sf('deadline',this.value)"></div>`;
  }else if(currentModalType==='debt'){
    const isUtang=tmpForm.type==='utang';
    const currOpts2=CURR.map(c=>`<option value="${c}"${(tmpForm.currency||'IDR')===c?' selected':''}>${c}</option>`).join('');
    body=`<div class="fg"><label>${isUtang?'Nama Kreditur (yang memberi utang)':'Nama Debitur (yang berutang ke kamu)'}</label><input class="inp" placeholder="Nama orang atau lembaga..." value="${esc(tmpForm.person||'')}" oninput="sf('person',this.value)"></div><div class="fg"><label>Nominal & Mata Uang</label><div style="display:grid;grid-template-columns:2fr 1fr;gap:6px"><input class="inp" type="number" placeholder="0" value="${tmpForm.amount||''}" oninput="sf('amount',this.value)"><select class="sel" onchange="sf('currency',this.value)">${currOpts2}</select></div></div><div class="fg"><label>Jatuh Tempo (opsional)</label><input class="inp" type="date" value="${tmpForm.dueDate||''}" oninput="sf('dueDate',this.value)"></div><div class="fg"><label>Keterangan</label><input class="inp" placeholder="Untuk keperluan apa..." value="${esc(tmpForm.note||'')}" oninput="sf('note',this.value)"></div>`;
  }else if(currentModalType==='inv'){
    const invTypeOpts=INV_TYPES.map(t=>`<option value="${t}"${(tmpForm.type||'Saham')===t?' selected':''}>${t}</option>`).join('');
    body=`<div class="fg"><label>Nama Aset</label><input class="inp" placeholder="BBCA, Bitcoin, Reksa Dana XYZ..." value="${esc(tmpForm.name||'')}" oninput="sf('name',this.value)"></div><div class="fg"><label>Jenis Investasi</label><select class="sel" onchange="sf('type',this.value)">${invTypeOpts}</select></div><div class="fg"><label>Modal / Harga Beli (IDR)</label><input class="inp" type="number" placeholder="0" value="${tmpForm.buyAmount||''}" oninput="sf('buyAmount',this.value)"></div><div class="fg"><label>Nilai Sekarang (IDR)</label><input class="inp" type="number" placeholder="0" value="${tmpForm.currentValue||''}" oninput="sf('currentValue',this.value)"></div><div class="fg"><label>Catatan (opsional)</label><input class="inp" placeholder="Platform, jumlah lot, dll..." value="${esc(tmpForm.note||'')}" oninput="sf('note',this.value)"></div>`;
  }else if(currentModalType==='rates'){
    const rateFields=CURR.filter(c=>c!=='IDR').map(c=>`<div class="fg"><label>1 ${c} = Rp</label><input class="inp" type="number" value="${tmpRates[c] !== undefined ? tmpRates[c] : rates[c]}" oninput="sfr('${c}',this.value)"></div>`).join('');
    body=`<p class="muted" style="margin-bottom:10px">Sesuaikan kurs untuk konversi otomatis ke IDR</p>${rateFields}`;
  }
...
function saveModal(){
  if(currentModalType==='tx'){
    ...
    const item={type:tmpForm.type||'expense',amount:amt,amountIDR:amtIDR,currency:tmpForm.currency||'IDR',category:cat,description:tmpForm.description,date:tmpForm.date||today(),note:tmpForm.note||''};
    if(editId){const tx=getTx();setTx(tx.map(t=>t.id===editId?{...t,...item}:t));}
    else setTx([{id:uid(),...item},...getTx()]);
  }else if(currentModalType==='budget'){
    ...
    if(editId){const debts=getDebts();setDebts(debts.map(d=>d.id===editId?{...d,...baseItem}:d));}
    ...
    if(editId){const invs=getInv();setInv(invs.map(i=>i.id===editId?{...i,...item}:i));}
...
// ==================== DELETE/UPDATE ====================
function deleteTx(id){if(confirm('Hapus transaksi ini?'))setTx(getTx().filter(t=>t.id!==id))}
...
function exportCSV(){
  ...
  const h=['Tanggal','Tipe','Kategori','Deskripsi','Nominal','Mata Uang','Nominal IDR','Catatan'];
  const rows=allTx.map(t=>[t.date,t.type==='income'?'Pemasukan':'Pengeluaran',CATS.find(c=>c.id===t.category)?.label||t.category,`"${(t.description||'').replace(/"/g,'""')}"`,t.amount,t.currency,Math.round(t.amountIDR),`"${(t.note||'').replace(/"/g,'""')}"`]);
